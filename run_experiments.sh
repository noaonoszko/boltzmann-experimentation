...
for same_model in "--same-model-init"; do
    for compression in 1 10 100 1000; do
        ...
        boltz run densenet --num-communication-rounds 23437 --gpu "$run_num" --only-train miners $same_model --model-kwargs '{"norm": "group"}' --compression-factors $compression > "${run_dir}/metrics.log" 2>&1 &
